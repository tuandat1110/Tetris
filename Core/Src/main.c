...
int main(void)
{
  ...
  //__HAL_RCC_RNG_CLK_ENABLE();      // Enable clock RNG
   //hrng.Instance = RNG;             // Khởi tạo RNG
   //hrng.Init.ClockErrorDetection = DISABLE;
   //HAL_RNG_Init(&hrng);
  while (1)
  {
    /* USER CODE END WHILE */
...
static void MX_GPIO_Init(void)
{
  ...
  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(GPIOC, GPIO_PIN_2|GPIO_PIN_10, GPIO_PIN_RESET);
  ...
  /*Configure GPIO pins : SPI5_NCS_Pin PC10 */
  GPIO_InitStruct.Pin = SPI5_NCS_Pin|GPIO_PIN_10;
  GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
